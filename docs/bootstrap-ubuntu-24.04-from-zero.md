...
24.04 must complete this entire runbook on a fresh VPS, survive a reboot, and pass the full
qualification scan in step 12 before it is marked supported.
...
A completed full scan remains required before this experimental Ubuntu runbook can be marked
supported. After qualification, it does not need to be repeated for each later VPS unless one
of the conditions above changes.
