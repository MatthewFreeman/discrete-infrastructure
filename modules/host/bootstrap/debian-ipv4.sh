...
configure_host_baseline() {
    ...
    bash "${HOST_ROOT}/scripts/configure-ipv4-only.sh"

    log "Configuring client-only time synchronization"
    bash "${HOST_ROOT}/scripts/configure-time-sync.sh"
}
...
verify_no_ipv6_listener() {
    bash "${HOST_ROOT}/scripts/check-ipv6-listeners.sh" strict >/dev/null
}

verify_prepare_ipv6_listener_state() {
    PREPARE_IPV6_LISTENER_STATE="$(
        bash "${HOST_ROOT}/scripts/check-ipv6-listeners.sh" prepare
    )" || return 1
}
...
ensure_admin_user() {
    ...
    ADMIN_USER="${ADMIN_USER}" \
        bash "${HOST_ROOT}/bootstrap/create-admin.sh"
...
build_bootstrap_nftables_config() {
    local source_config="${HOST_ROOT}/configs/nftables/nftables.conf"
...
install_bootstrap_firewall() (
    ...
    bash "${HOST_ROOT}/scripts/apply-nftables.sh" --check "${temporary_config}"

    install -m 0644 "${temporary_config}" /etc/nftables.conf
    systemctl enable nftables >/dev/null

    if systemctl is-active --quiet nftables; then
        bash "${HOST_ROOT}/scripts/apply-nftables.sh" --apply /etc/nftables.conf
    else
...
build_bootstrap_fail2ban_config() {
    local source_config="${HOST_ROOT}/configs/fail2ban/jail.local"
    local output_config="$1"
...
install_bootstrap_fail2ban() (
    ...
    install -m 0644 "${HOST_ROOT}/configs/fail2ban/fail2ban.local" \
        /etc/fail2ban/fail2ban.local
    install -m 0644 "${temporary_config}" /etc/fail2ban/jail.local

    fail2ban-client -t
    bash "${HOST_ROOT}/scripts/restart-fail2ban.sh"
...
apply_prepare_components() {
    ...
    bash "${HOST_ROOT}/scripts/verify.sh" nftables-bootstrap
    bash "${HOST_ROOT}/scripts/verify.sh" fail2ban-bootstrap
}
...
finalize_ssh() {
    ...
    if bash "${HOST_ROOT}/scripts/apply-config.sh" ssh \
...
finalize() {
    ...
    bash "${HOST_ROOT}/scripts/apply-config.sh" nftables
    bash "${HOST_ROOT}/scripts/apply-config.sh" fail2ban

    remove_ufw
    verify_no_ufw

    log "Running complete IPv4-only verification"
    bash "${HOST_ROOT}/scripts/verify.sh" all
...
status_check() {
    ...
    if output="$(bash "${HOST_ROOT}/scripts/verify.sh" "${mode}" 2>&1)"; then
