...
const source=pay+'/build/native-test/paged/run-CzQ3ab';
...
try {
 ...
 record('account publication',{registered:account.result?.registered,blockHeight:account.result?.blockHeight,txIndex:account.result?.txIndex,rpcCode:account.error?.code,applicationCode:account.error?.data?.application_code});
 const direct=await rpc(wp,'getTransaction',{transactionHash:'6cfc013b827d659e7103e164f059dd11f7529ac1a1de71367b885052252f8da8'},true);
 record('direct registration transaction',{hash:direct.result?.transaction?.transactionHash,blockIndex:direct.result?.transaction?.blockIndex,state:direct.result?.transaction?.state,confirmations:direct.result?.transaction?.confirmations,errorCode:direct.error?.code});
 ...
 for(let h=48;h<=tip;h++){
   ...
   if(tx.error || tx.result?.items?.some(b=>b.transactions.length)) record('transactions at height',{height:h,transactions:tx.result?.items?.flatMap(b=>b.transactions.map(t=>({hash:t.transactionHash,blockIndex:t.blockIndex,fee:t.fee,state:t.state}))),errorCode:tx.error?.code});
 }
