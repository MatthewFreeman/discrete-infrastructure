...
try {
 ...
 const registry=await readPagedRegistry(nativeClient,attest);assert.equal(registry.length,10000);
 ...
 try{for(const entry of registry){if(assigned.has(entry.routingT))continue;const t=entry.routingT,now=Date.now(),requestId='combined_seed_request_'+String(t).padStart(8,'0');
  ...
  assert.equal(created.kind,'created');journal.prepare(requestId,registryHash(registry.slice(0,t-1)),t-1,attest.accountNumber,now);journal.commit(requestId,entry,now);
  store.assignInvoiceDeposit(created.invoice.id,requestId,t,entry.address,now);
 }
 assert.equal(journal.list().length,10000);assert(journal.list().every(r=>r.state==='committed'));
 }finally{journal.close();}
 assert.equal(store.listAssignedInvoices().length,10000);
 record('seeded full invoice and allocation journals using validated native addresses',{seeded:10000-existing.length,retainedOriginal:existing.length,elapsedMs:Math.round(performance.now()-seedStart),nativeAllocationsDuringSeeding:0});
 ...
 record('T10000 wallet outage and wallet/worker reopen preserve payment without duplicate events or wrong-invoice credits',{registryCount:10000,invoiceRows:10000,unpaidNeighbors:3,originalConfirmedAtomic:'12345',targetEventCount:events.length});
 // A distinct native transaction credits twenty seeded high-index invoices.
 const batch=store.listAssignedInvoices().filter(r=>r.depositT>=9000&&r.depositT<9020);assert.equal(batch.length,20);assert(batch.every(r=>r.confirmedAtomic===0n&&r.amountAtomic===1n));
 ...
 record('20 confirmed native recipients and20 stable full-journal scan cycles',{invoiceRows:10000,elapsedMs:Math.round(performance.now()-steadyStart),minCycleMs:Math.min(...cycleMs),maxCycleMs:Math.max(...cycleMs),meanCycleMs:Math.round(cycleMs.reduce((a,b)=>a+b,0)/cycleMs.length),duplicateEvents:0});
 const createNext=async(expected)=>{const started=performance.now();const r=await fetch('http://127.0.0.1:'+gp+'/v1/invoices',{method:'POST',headers:{authorization:'Bearer '+h.merchantToken,'content-type':'application/json','idempotency-key':'combined-native-next-invoice'},body:JSON.stringify({amount_atomic:'1',expires_in_seconds:3600,required_confirmations:2}),signal:AbortSignal.timeout(15000)});assert.equal(r.status,expected);return {id:(await r.json()).invoice.id,elapsedMs:Math.round(performance.now()-started)};};
 const next=await createNext(201);assert.equal(store.getInvoiceById(next.id).depositT,10001);assert.equal((await nativeClient.getDepositScheme()).depositCount,10001);assert.equal(store.listAssignedInvoices().length,10001);
 await closeApps();const reopenMs=await startApps();assert.equal((await createNext(200)).id,next.id);assert.equal((await nativeClient.getDepositScheme()).depositCount,10001);
 record('actual native merchant allocation10001 with full journal and cold reopen replay',{allocationMs:next.elapsedMs,reopenMs,invoiceRows:10001});
 await closeApps();
 result.result='PASS';
}catch(e){result.result='FAIL';result.error=e.message;console.error(e.message);process.exitCode=1;}
finally{if(mining)await stopMine().catch(()=>{});await worker?.close();await gateway?.close();await facade?.close();await web?.close();if(receiver?.listening)await new Promise(r=>receiver.close(r));store?.close();await Promise.allSettled([...children].map(stop));await writeFile(dir+'/evidence.json',JSON.stringify(result,null,2),{mode:0o600});console.log('Evidence:',dir+'/evidence.json');}
